...
\begin{figure}[H]
  ...
  \caption[Maximum deviation as a function of the bin boundary energy.]{%
    Maximum deviation observed, $\delta_{\max}$, for each admissible
  boundary position within the calibrated 20 to 40~keV window.
  }
  \label{fig:scan}
\end{figure}
The minimum, $0.07\%$, occurs at $24.5$~keV. This is the calibrated energy of the channel at which N-25 ends. At this boundary N-25 contributes to the lower bin alone and fixes $w_1$ by itself, while N-30 and N-40 both contribute to the upper bin and determine $w_2$. Every quality is used, and no weight is constrained twice. Above this boundary, in the region marked by the grey band, N-25 and N-30 both contribute exclusively to the lower bin, which includes the natural equal-width division at 30~keV. For these cases, the two qualities have incompatible values of $w_1$, $93.6$ and $139.0$~pSv/count, respectively. The solution is therefore a compromise between the two with a represented maximum deviation of $23.6\%$. The bins are therefore defined as $N_1 \in \left[20,25\right]$~keV and $N_2 \in \left]25,40\right]$~keV, and the resulting count rates are listed in Table~\ref{tab:matrix}.
...
With these values, solving Eq.~\ref{eq:lsq} gives: 
\begin{equation}
