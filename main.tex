...
To detect the scintillation light and convert it into an electrical signal, a SiPM from Broadcom (AFBR-S4N66P014M) was optically coupled to the scintillator. Its $6\times6$~mm\textsuperscript{2} active area matches the scintillator cross section, maximizing light collection.

A SiPM consists of a parallel array of avalanche photodiodes operated in Geiger mode. Each cell is reverse biased above its breakdown voltage, so a single absorbed photon is sufficient to trigger a self-sustaining avalanche. The photons produced by a scintillation event fire many cells simultaneously, each of which contributes one identical pulse. The summed output current is proportional to the number of fired cells, and therefore to the energy deposited in the scintillator. As represented in Fig.~\ref{fig:sipm} (a), the photon detection efficiency (PDE) of the AFBR-S4N66P014M peaks near the emission maximum of BC-408, ensuring efficient detection of the scintillation light. The number of available microcells (22\,428) fulfills the requirements given the photons produced per scintillation event. The SiPM has a breakdown voltage of 33~V and supports overvoltages of up to 16~V.
\begin{figure}[H]
  \centering
  \newlength{\panelheight}
  \setlength{\panelheight}{5.2cm}% adjust once, both panels follow
  \subfloat[]{\label{fig:sipm_pde}%
    \includegraphics[height=\panelheight]{images/Emission.png}}
  \hfill
  \subfloat[]{\label{fig:bias}%
    \resizebox{!}{\panelheight}{\input{figures/MAX1932}}}
  \caption[SiPM spectral response and bias supply.]{%
    \textbf{(a)} caption
    \textbf{(b)} MAX1932 feedback network with the NTC thermistor $R_T$ for temperature compensation of the SiPM bias.
  }
  \label{fig:sipm}
\end{figure}
...
As for the Compton edge of the 122 keV line of $^{57}$Co, the feature was extracted as the channel of maximum counts. As introduced in Section~\ref{sec:compton_scattering}, this corresponds to the maximum energy transfer at the scattering angle of 180$^\circ$. Its uncertainty is larger than that of the $^{241}$Am photopeak, due to the less defined nature of the feature. Swiderski \emph{et al}.~\cite{122} have shown that, owing to the finite resolution of plastic, the Compton edge may not coincide with the counts maximum. The relative shift is larger at lower energies and can be resolved with the previously mentioned coincidence techniques using the backscatter peak~\cite{122}. Within these limitations, and as represented in Fig.~\ref{fig:energy_calibration} (b), the extracted channel, $C_E$, falls, within its own uncertainty, inside the band at which the calibration predicts the 39.46 keV edge. The calibration therefore correctly locates an independent energy reference. The edge was nevertheless excluded from the calibration itself, since the 40 kVp \emph{Bremsstrahlung} endpoint provides a more reliable point at a close energy.
