...
\begin{table}[H]
  ...
  \end{tabular}
\end{table}
To extend the calibration beyond the range of the X-ray tube, spectra of the reference sources were acquired with the plastic scintillator over 90 min intervals. Of the three sources, the only features available are the $^{241}$Am photopeak at 59.5 keV, and the Compton edge of the 122 keV line of $^{57}$Co, at 39.46 keV, shown in Fig.~\ref{fig:energy_calibration} (a) and (b) respectively. The latter is treated as a validation of the \emph{Bremsstrahlung} method, as it falls within the energy range provided by the tube. The photopeak is, therefore, included as a calibration point at that energy.
\begin{figure}[H]
  \centering
  ...
  \end{minipage}
  \caption[\emph{Bremsstrahlung}-Based Energy Calibration.]{%
    \textbf{(a)} caption
    \textbf{(b)} caption
    \textbf{(c)} caption
    }
...
The mean and standard deviation were computed for each of the seven relevant voltages, from 20 to 50 kVp, and for the $^{241}$Am centroid. A weighted least-squares fit was applied to account for the differences in uncertainty across the energy range and is shown in Fig.~\ref{fig:energy_calibration} (c). For the \emph{Bremsstrahlung} endpoints, the uncertainty at the lower voltages is dominated by counting statistics. These energies lie in the boundary at which photoelectric absorption ceases to dominate over Compton scattering in BC408~\cite{124}. The near-invisible error bars between 30 and 40 keV indicate that in this region both the photoelectric cross-section and the photon fluence are sufficient for the endpoint to be determined with good precision. Although the 45 keV error bar remains small, both the 45 and 50 keV endpoints fall below the calibration line, beyond their uncertainties. This underestimation reflects the reduced photoelectric cross-section, which drops below 0.5\% in the scintillator at 50 keV. As full energy transfer becomes less probable, the spectrum compresses toward a lower and less reliable endpoint. Additionally, pileup further distorts the spectral shape near the abscissa. The pileup distortion is visible as a non-linear tail in the higher energy spectra of Fig.~\ref{fig:bremss_calibration} (a), which is not characteristic of filtered \emph{Bremsstrahlung}. At higher count rates, the baseline does not recover between successive pulses, causing them to superimpose and producing the observed tail. These are the fundamental limitations of the endpoint method at higher energies, as the associated uncertainty is expected to grow accordingly. In contrast, the $^{241}$Am centroid falls on the calibration line with an uncertainty smaller than that of the highest-energy endpoint estimate. Although the peak is not as sharp as that obtained with the CsI crystal in Section~\ref{sec:frontend-validation}, the centroid position is reproducible across acquisitions.
